...
export const segmentPropsSchema = z.object({
  ...
  dotPulseSpeed: z
    ...
    .min(0)
    .max(500)
    .describe("Dot flash speed (0=static, 100=default)"),
  routeGlow: z
    .number()
    .min(0)
    .max(200)
  ...
  mapFileEnd: z
    .string()
    .describe("Second map PNG for tile transition (empty=no transition)"),
...
export const RouteSegmentVideo: React.FC<RouteSegmentVideoProps> = ({
  ...
  const cameraZoom = cameraEffect
    ? cameraEffect.startZoom +
      (cameraEffect.endZoom - cameraEffect.startZoom) * progress
    : 1;
